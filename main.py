...
from display import start_display, set_speed, set_no_signal
...
# Start the 7-segment display thread (begins in "loading" spinner mode by default)
start_display()
...
while True:
    ...
    if result['fix_type'] >= MIN_FIX_TYPE and result['sats'] >= MIN_SATS:
        ...
        if speed_limit is not None:
            ...
            # On a road but no maxspeed tag — optional: show last known or fallback
            # For now, keep showing last valid limit (common real-world behavior)
            print(f"No speed limit tag found (closest road: {highway_type}, dist: {dist:.1f}m)")
            # You could choose to set_no_signal() here if you want spinner instead

    else:
        # Poor or no GPS fix — show loading animation
        print("Waiting for GPS fix "
              f"(Fix: {result['fix_type']}, Sats: {result['sats']})...")
        set_no_signal()
        last_speed_limit = None  # Reset so next valid limit triggers update

    # Loop delay — 1 Hz is plenty for speed limit changes
    time.sleep(1)